...
#define LOG_ERROR(fmt, ...) \
        do { fprintf(stderr, "[LOOPBACKCAPTURE] %s:%d:%s(): " fmt "\n", __FILE__, \
                                __LINE__, __func__, __VA_ARGS__); (void)fflush(stderr); } while (0)

#define LOG_INFO(fmt, ...) \
        do { fprintf(stdout, "[LOOPBACKCAPTURE] %s:%d:%s(): " fmt "\n", __FILE__, \
                                __LINE__, __func__, __VA_ARGS__); (void)fflush(stdout); } while (0)
...
public:
    ...
    //HRESULT SetParam(LoopbackParam* p) { lparam = p; return S_OK; }
...
public:
    LoopbackParam lparam;  // this is not used here
};
